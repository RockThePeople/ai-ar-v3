...
- 🔴 **웨이브마다 새 브랜치를 판다** (D27-c). 브랜치를 재사용하지 않는다.
  머지된 브랜치에 이어서 푸시하면 **머지 시점 뒤의 커밋이 상대에게 안 보인다** —
  PR #6 에서 3090 의 W21 산출물이 그렇게 빠졌고, **같은 자리가 두 번째**다.
  이름은 `<세션>/<웨이브>-<주제>` 로 한다 (예: `contract/chunk4-d75`).
