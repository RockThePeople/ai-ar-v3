...
## W27① 용 — `<GEN_HOST>` 서빙 확정

**asset_id 는 `v3-moto-b`** (대조군으로 `v3-dragon-c` 도 같이 선다).

```
GET /v2/assets/v3-moto-b/manifest.v1.json      → 200 · 청크 376 · contract v4
GET /v2/assets/v3-moto-b/chunks/<key>.v1.cbin  → 200 · 판본 v4
```

청크 URI 는 매니페스트의 `chunks[key].uri` 를 그대로 쓴다.

### 맥북이 할 판정을 미리 해 뒀다

HTTP 매니페스트의 `hash` 와 **리포 사본**(= APK 에 들어가는 바이트)을 전량 대조:

| 자산 | 일치 | 불일치 |
|---|---|---|
| `v3-moto-b` | **376 / 376** | 0 |
| `v3-dragon-c` | **485 / 485** | 0 |

⇒ 전송 판정 장치가 선다. 생성물(`v3-j-…`, duck 989청크)은 APK 에 사본이 없어
   이 대조가 성립하지 않는다 — **W27① 은 `v3-moto-b` 로 해야 한다.**

### 🔴 재시작에 안 죽게 고정했다

전에는 저장소 루트를 **기동 명령줄에만** 실었다. 그러면 누가 그냥
`./server/run-8083.sh` 로 재시작하는 순간 리포 자산이 통째로 안 보인다
(`모르는 자산이다: 'v3-moto-b'`). W27 이 이 자산에 걸려 있으니 **한 번의 재시작으로
남의 웨이브가 멈춘다.**

이제 `run-8083.sh` 가 `~/.ai-ar-v3-server.env`(600, 리포 밖)를 읽는다. 값은 그 파일에만
있고 리포·로그에는 없다 (§7). 환경변수 없이 맨몸으로 재시작해 위 수치를 다시 확인했다.
