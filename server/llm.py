...
D26 매핑표 (정본은 `server/README.md`):

    op                소비자                        비고
    ────────────────  ────────────────────────────  ─────────────────────────────
    replace_region    assemble / VoxHammer          호박 머리는 assemble 로 달성됨
    recolor           recolor 경로 (D24)            복셀 격자 미경유 · GPU 불필요
    add               🔴 **VoxHammer 전용**          assemble 로 보내면 **조용히
                                                    아무 일도 안 일어난다** (D22 ①)
    remove            미정                          아직 0회

🔴 방어는 **소비자 쪽**이다. 소비자가 자기가 처리 못 하는 op 를 받으면 거부한다.
   여기에 분기를 넣지 않는다.
...
OPS = ("replace_region", "recolor", "add", "remove")
...
EDIT_SPEC_SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        "op": {
            ...
            "enum": list(OPS),
            "description": (
                "replace_region: 마스크 영역의 것을 **다른 물체**로 바꾼다(형태가 바뀐다). "
                "recolor: 형태는 그대로 두고 **색·재질만** 바꾼다. "
...
## op 고르기
- replace_region  그 자리의 것을 **다른 물체**로 바꾼다. 형태가 바뀐다.
                  "머리를 호박으로", "머리를 해골로"
- recolor         **형태는 그대로**, 색·재질만 바꾼다.
                  "머리만 빨갛게", "몸통을 파란색으로", "금속 재질로"
                  ⚠️ 형태가 바뀌면 replace_region 이다. 색만이면 recolor 다.
...
- 결과물의 묘사다. 동사가 아니라 명사구로.
  "머리를 주황색 할로윈 호박으로 바꿔" → "주황색 할로윈 호박"   (replace_region)
  "머리만 빨갛게"                     → "빨간색"               (recolor)
  "머리를 3개로 만들어"               → "머리 3개"             (add)
