...
def test_surface_source_is_rejected():
    ...
    with pytest.raises(NotSlatCoords):
        build_head3_mask(_necked_asset(), source=SURFACE)
...
def test_slat_label_still_checked_against_manifest():
    ...
    with pytest.raises(NotSlatCoords) as e:
        build_head3_mask(asset, source=SLAT, manifest=manifest)
...
def test_matching_manifest_passes():
    ...
    spec = build_head3_mask(
        asset, source=SLAT, manifest={"voxel_count_total": len(asset), "chunks": []}
    )
...
def test_cut_is_above_the_neck_minimum():
    ...
    spec = build_head3_mask(asset, source=SLAT)
...
def test_mask_is_mostly_empty_space():
    ...
    spec = build_head3_mask(_necked_asset(), source=SLAT)
...
def test_mask_widens_for_three_heads():
    ...
    asset = _necked_asset()
    spec = build_head3_mask(asset, source=SLAT, width_multiple=1.0)
...
def test_spec_carries_grid_source_and_fingerprint():
    ...
    d = build_head3_mask(_necked_asset(), source=SLAT).as_dict()
...
def test_empty_occupancy_is_rejected():
    with pytest.raises(ValueError):
        build_head3_mask(np.zeros((0, 3), dtype=np.int64), source=SLAT)
