...
namespace DeltaContract.EditorTools
{
    public static class V3AppBuild
    {
        ...
        static void BuildScene(string caseFile)
        {
            ...
            // 🔴 W27① — 서버 주소는 **환경변수에서만** 온다. 리포·코드에 값을 두지 않는다 (§7).
            //    비어 있으면 앱이 옛 경로(StreamingAssets)를 쓴다 — 전송만 갈아끼우는 것이
            //    목적이라 두 경로가 공존해야 원인을 가를 수 있다.
            var host = Environment.GetEnvironmentVariable("GEN_HOST") ?? "";
            var port = Environment.GetEnvironmentVariable("GEN_PORT") ?? "8083";
            app.ServerUrl = string.IsNullOrEmpty(host) ? "" : $"http://{host}:{port}";
            app.AssetId = Environment.GetEnvironmentVariable("ASSET_ID_REMOTE") ?? "v3-moto-b";
            Debug.Log($"[V3AppBuild] 자산 출처: {(string.IsNullOrEmpty(app.ServerUrl) ? "APK" : "HTTP")} · id={app.AssetId}");
