...
namespace DeltaContract
{
    public sealed class ArPlacement : MonoBehaviour
    {
        ...
        void LogDiag()
        {
            // 🔴 옛 판별식은 이름 매칭이라 HandheldARInputDevice 를 놓쳐 **xr=0 으로 보였다**
            //    — 장치가 없는 게 아니라 검사가 틀린 것이었다. 이름을 **덤프**해 눈으로 본다.
            int xr = 0;
            var names = new System.Text.StringBuilder();
            foreach (var d in InputSystem.devices)
            {
                bool isXr = d is UnityEngine.InputSystem.XR.XRHMD
                            || d.description.interfaceName == "XRInput"
                            || d.layout.Contains("XR") || d.layout.Contains("Handheld");
                if (isXr) { xr++; names.Append(d.name).Append('(').Append(d.layout).Append(") "); }
            }
